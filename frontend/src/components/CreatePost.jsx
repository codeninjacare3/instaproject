...
const {posts} = useSelector(store=>store.post);
    ...
    const createPostHandler = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                dispach(setPosts([res.data.post, ...posts]));//[1]-> [1,2]-> total 2 posts
    ...
    return (
        <Dialog open={open}>
            <DialogContent onInteractOutside={() => setOpen(false)}>
                ...
                <Button onClick={() => imageRef.current.click()} className='w-fit mx-auto bg-[#0095f6] hover:bg-[#258bcf]'>Select from PC</Button>
                {
                    imagePreview && (
                        loading ? (
                            <Button>
